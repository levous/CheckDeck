...
- (id)initWithDbName:(NSString *)dbName{
    ...
    //managedObjectContext = [simpleStore managedObjectContext];
      ...
      CDLog(@"%@ DB not present, will be created upon save", dbName);
      [self initializeDatabaseWithStartupData];
      
		}
    
    SimpleStore *store = [SimpleStore storeWithPath:dbFilePath];
...
- (BOOL)wipeDatabaseFromDevice{
  ...
  if( error != nil ) {
    CDLog( @"Attempted to wipe database file but received error: %@", [error localizedDescription], nil );
    success = NO;
  }
